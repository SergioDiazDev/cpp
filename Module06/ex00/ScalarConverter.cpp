...
char ScalarConverter::getType(std::string const &literal)
{
	...
	else if (literal.find(".") == no && ScalarConverter::is_number(literal))
		type = 'i';
...
void ScalarConverter::error(void)
{
...
bool ScalarConverter::is_number(std::string const &literal)
{
